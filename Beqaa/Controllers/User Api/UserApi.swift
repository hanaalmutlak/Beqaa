...
class UserApi {
    ...
    static func addUser(uid:String,email:String,name:String,phoneNumber:String,city:String,completion: @escaping (Bool) -> Void) {
        let db = Firestore.firestore()
        let refUser = db.collection("Users")
        refUser.document(uid).setData(Users.createUser(email: email, name: name, phoneNumber: phoneNumber, city: city))
