...
class RegistrtionViewController: UIViewController {
    ...
    @IBAction func RegistrationTapped(_ sender: Any) {
        ...
        Auth.auth().createUser(withEmail: EmailTF.text ?? "", password: PasswordTF.text ?? "") { user, error in
            ...
            if let error = error {
                ...
                UserApi.addUser(uid: user?.user.uid ?? "", email: self.EmailTF.text ?? "", name: self.UserNameText.text ?? "", phoneNumber: self.PhoneNamberTF.text ?? "", city: self.CityTF.text ?? "") { result  in
                    if result {
                        print("done Saving")
                    }
                }
